...
def hisse_secici(key_prefix, varsayilan="THYAO.IS"):
    ...
    if mod == "Popüler Listeden Seç":
        ...
        sorgu = st.text_input("Şirket adı veya sembol yazın", key=f"{key_prefix}_q")
        if not sorgu:
            return None
...
def hakkinda_sayfasi():
    ...
    with col1:
        # İstenen özellik: Kadın avatar varsayılanı ve gerçek fotoğraf yüklenebilir alan
    ...
    with col2:
        ...
        # İstenen özellik: Mail, LinkedIn ve GitHub yanında gerçek ikonlar
        st.markdown("""
...
""")
    ...
    # İstenen özellik: Gerçek CV yükleme ve indirme alanı (Demo yerine aktif yükleme)
